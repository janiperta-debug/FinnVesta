...
def calculate_annual_depreciation(replacement_value: float) -> float:
    ...
    """
    return float(replacement_value) * 0.0175
...
def calculate_repair_debt(
    ...
    if condition_score >= 0.75:
        ...
        target_value = float(replacement_value) * 0.75
        kptarve = max(0, target_value - float(technical_value))
        pptarve = 0.0
    else:
        # Poor condition - improvement needed to upgrade to 120%
        target_value = float(replacement_value) * 1.20
        kptarve = 0.0
        pptarve = max(0, target_value - float(technical_value))
...
def calculate_valuation_from_assessment(
    area_m2: float,
    cost_per_m2: float,
    construction_year: int,
    pka_score: float,
    ...
    """Calculate valuation based on observed condition (PKA score).
    
    Overrides theoretical age-based depreciation with observed condition.
    
    Args:
        area_m2: Building area in square meters
        cost_per_m2: Construction cost per square meter (€/m²)
        construction_year: Year the building was constructed
        pka_score: Observed condition score (1-5 scale)
        reference_year: Reference year (defaults to current year)
    ...
    # Convert PKA (1-5) to Condition Score % (0-1)
    # Mapping: 5 -> 1.0 (100%), 1 -> 0.0 (0%)
    # Formula: (score - 1) / 4
    # Example: 3.0 -> 2/4 = 0.5 (50%)
    kla = max(0.0, min(1.0, (float(pka_score) - 1.0) / 4.0))
    
    # Calculate Technical Value based on Observed Condition
    # TeknA = JHA * kla
    tekna = jha * kla
    
    # Calculate repair debt based on this observed condition
    repair_debt = calculate_repair_debt(kla, jha, tekna)
    
    return {
        'replacement_value': jha,
        'annual_depreciation': 0.0,  # Not relevant when overriding with observed condition
        'building_age': age,
        'technical_value': tekna,
        'condition_score': kla,
        'kptarve': repair_debt['kptarve'],
